...
const DEVICE_TYPES = [
  {
    ...
    label: "Mobile / Tablet",
    icon: Smartphone,
    brands: ["Samsung", "Apple", "OnePlus", "Xiaomi", "Oppo", "Vivo", "Realme"],
  },
  {
    value: "laptop",
    label: "Laptop",
  ...
  },
  {
    ...
    label: "Desktop PC",
    icon: Monitor,
    brands: ["Custom Build", "Dell", "HP", "Lenovo", "Asus"],
  },
  {
    value: "console",
...
const URGENCY = [
  {
    value: "low",
    label: "Low priority",
    desc: "Happy to wait 5–7 days",
    color: "border-green-200 bg-green-50 text-green-700",
  },
  {
    value: "medium",
    label: "Medium priority",
    desc: "Need it within 3–4 days",
    color: "border-amber-200 bg-amber-50 text-amber-700",
  },
  {
    value: "high",
    label: "Urgent",
    desc: "Need it within 1–2 days",
...
export function RaiseTicket() {
  ...
  const setAddr = (key, val) =>
    setForm((p) => ({
      ...p,
      pickupAddress: { ...p.pickupAddress, [key]: val },
    }));

  const canNext = () => {
    if (step === 0)
      return form.deviceType && form.deviceBrand && form.deviceModel;
    if (step === 1)
      return form.issueTitle.length >= 5 && form.issueDescription.length >= 20;
    return true;
  };
    ...
    <CustomerLayout title="Raise a Repair Ticket">
      <div className="max-w-2xl mx-auto">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {DEVICE_TYPES.map(({ value, label, icon: Icon }) => (
                  ...
                  >
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center ${form.deviceType === value ? "bg-brand-100" : "bg-surface-100"}`}
                    >
                      <Icon
                        size={22}
                        className={
                    ...
                    <span
                      className={`text-sm font-medium ${form.deviceType === value ? "text-brand-700" : "text-surface-700"}`}
                    >
                      {label}
                    </span>
                  </button>
                ...
                <div className="grid grid-cols-2 gap-4 animate-fade-in">
                  ...
                  <Field label="Serial / IMEI" className="col-span-2">
                    <input
                      ...
                      placeholder="Optional — helps providers identify your device"
                      className="input"
                    />
                  </Field>
                </div>
          ...
          {step === 1 && (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <p className="text-sm text-surface-500">
                  Be specific — better descriptions attract more accurate bids.
                </p>
              </div>
              ...
              <Field label="Issue title" required>
                <input
                  ...
                  placeholder="e.g. Screen cracked and touch not responding"
                  className="input"
                  maxLength={100}
                />
              ...
              <Field label="Detailed description" required>
                ...
                />
                <p
                  className={`text-[10px] mt-1 ${form.issueDescription.length < 20 ? "text-red-400" : "text-surface-400"}`}
                >
              ...
              <Field label="Urgency">
                <div className="flex flex-col gap-2">
                  {URGENCY.map((u) => (
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${form.urgency === u.value ? "border-current" : "border-surface-300"}`}
                      >
          ...
          {step === 2 && (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <p className="text-sm text-surface-500">
                  Help providers understand your budget and logistics.
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    },
                    {
                      v: "drop_off",
                      l: "Drop off at shop",
                      d: "You visit the shop",
                    },
                  ].map((opt) => (
                    <button
                      key={opt.v}
                      onClick={() => set("preferredHandover", opt.v)}
                      type="button"
                    ...
                    >
                      <p
                        className={`text-sm font-medium ${form.preferredHandover === opt.v ? "text-brand-700" : "text-surface-700"}`}
                      >
                        {opt.l}
                      </p>
              ...
              {form.preferredHandover === "pickup_delivery" && (
                <div className="flex flex-col gap-3 animate-fade-in">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <Field label="Pincode">
                      <input
                        value={form.pickupAddress.zipCode}
          ...
          {step === 3 && (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <p className="text-sm text-surface-500">
                  Everything looks good? Submit to start receiving bids.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3">
                {[
                  {
                    label: "Device",
                    value: `${form.deviceBrand} ${form.deviceModel} (${form.deviceType})`,
                    step: 0,
                  },
                  ...
                  {
                    label: "Urgency",
                    value: URGENCY.find((u) => u.value === form.urgency)?.label,
                    step: 1,
                  },
                  {
                    label: "Budget",
                  ...
                  <div
                    key={row.label}
                    className="flex items-start justify-between gap-4 py-2.5 border-b border-surface-100 last:border-0"
                  >
                    <span className="text-sm text-surface-500 w-24 flex-shrink-0">
                      {row.label}
                    </span>
                    <span className="text-sm text-surface-800 font-medium flex-1 text-right">
                      {row.value}
                    </span>
                    <button
              ...
              <div className="p-3 bg-brand-50 rounded-xl border border-brand-100">
                <p className="text-xs text-brand-700 leading-relaxed">
                  <strong>What happens next:</strong> Your ticket will be
                  visible to approved providers. You'll receive bids within 24
                  hours. Once you accept a bid, the provider will coordinate
                  pickup.
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mt-4">
          <button
            onClick={() =>
              step > 0 ? setStep((s) => s - 1) : navigate("/customer/dashboard")
            }
