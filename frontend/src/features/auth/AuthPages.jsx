        ...
        <>
          <div>
            ...
            <p className="text-brand-200 text-sm leading-relaxed">
              Connect with trusted repair professionals near you. Get
              competitive bids, track your repair in real-time, and have your
              device returned to your door.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {features.map(({ icon: Icon, text }) => (
              <div
                key={text}
                className="bg-brand-900/50 rounded-xl p-3 flex items-center gap-2.5"
              >
                <Icon size={16} className="text-brand-300 flex-shrink-0" />
                <span className="text-brand-100 text-xs font-medium">
                  {text}
                </span>
              </div>
            ))}
      ...
      <div className="mt-6 p-4 bg-surface-100 rounded-xl">
        ...
        <div className="flex flex-col gap-1">
          {[
            ...
            >
              <span className="font-medium text-surface-700">{d.role}:</span>{" "}
              {d.email} / Demo@1234
            </button>
          ))}
        </div>
...
export function Register() {
  ...
  const toggleCat = (c) =>
    setForm((p) => ({
      ...p,
      serviceCategories: p.serviceCategories.includes(c)
        ? p.serviceCategories.filter((x) => x !== c)
        : [...p.serviceCategories, c],
    }));
      ...
      side={
        <>
          <div>
            ...
            <p className="text-brand-200 text-sm leading-relaxed">
              Whether you need a repair or run a repair shop, RepairLink makes
              the process seamless, transparent, and fair.
            </p>
          </div>
          <div className="space-y-3">
            {[
              "Post your problem for free",
              "Get competitive bids within 24h",
              "Track repair in real-time",
              "Doorstep pickup & delivery",
            ].map((f) => (
              <div
                key={f}
                className="flex items-center gap-2.5 text-brand-100 text-sm"
              >
                <CheckCircle
                  size={16}
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { v: "customer", l: "I need a repair", d: "Device owner" },
            { v: "provider", l: "I run a shop", d: "Repair business" },
          ].map((r) => (
            <button
              key={r.v}
              type="button"
            ...
            >
              <p
                className={`text-sm font-semibold ${form.role === r.v ? "text-brand-700" : "text-surface-800"}`}
              >
                {r.l}
              </p>
          ...
          {form.password && (
            <div className="flex gap-1 mt-1">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-1 flex-1 rounded-full transition-all ${i < strength ? ["bg-red-400", "bg-amber-400", "bg-amber-400", "bg-green-500"][strength - 1] : "bg-surface-200"}`}
                />
              ))}
            </div>
          )}
          ...
          <>
            <Field label="Shop name" required>
              <input
                type="text"
                value={form.shopName}
                placeholder="e.g. TechFix Chennai"
                onChange={(e) =>
