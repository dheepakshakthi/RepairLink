...
class TicketService {
  async createTicket(customerId, ticketData) {
    ...
    const ticket = await Ticket.create({
      ...
      customerId,
      biddingExpiresAt,
      statusHistory: [{ status: 'open', changedBy: customerId, changedAt: new Date() }]
    });

    // Schedule Bull job for bidding expiration
    await biddingExpireQueue.add({ ticketId: ticket._id }, { delay: 24 * 60 * 60 * 1000 });
  ...
  async getMarketplace(providerId, queryParams) {
    const { deviceType, urgency, budgetMin, budgetMax, page = 1, limit = 10, sort = 'createdAt_desc' } = queryParams;
  ...
  async getTicket(ticketId, user) {
    ...
    // Access control
    if (user.role === 'customer' && ticket.customerId._id.toString() !== user._id.toString()) {
      throw new ApiError(403, 'Access denied');
    }

    if (user.role === 'provider') {
      const isAssigned = ticket.assignedProviderId && ticket.assignedProviderId._id.toString() === user._id.toString();
      const hasBid = await Bid.exists({ ticketId, providerId: user._id });

      if (!isAssigned && !hasBid) {
        throw new ApiError(403, 'Access denied. You have not bid on or been assigned this ticket.');
      }
    }
  ...
  async cancelTicket(ticketId, customerId) {
    ...
    ticket.statusHistory.push({ status: 'cancelled', changedBy: customerId, changedAt: new Date() });

    await ticket.save();

    // Reject all pending bids
    await Bid.updateMany({ ticketId, status: 'pending' }, { status: 'declined' });
  ...
  async updateRepairStatus(ticketId, providerId, newStatus) {
    ...
    if (!ticket.assignedProviderId || ticket.assignedProviderId.toString() !== providerId.toString()) {
    ...
    if (!allowedNextStatuses || !allowedNextStatuses.includes(newStatus)) {
      throw new ApiError(400, `Invalid status transition from ${ticket.status} to ${newStatus}`);
    }

    ticket.status = newStatus;
    ticket.statusHistory.push({ status: newStatus, changedBy: providerId, changedAt: new Date() });
    ...
    try {
      getIo().to(`ticket:${ticketId}`).emit('ticket:status-updated', { ticketId, status: newStatus, updatedAt: new Date() });
  ...
  async addRepairLog(ticketId, providerId, logData) {
    ...
    if (!['device_received', 'in_repair'].includes(ticket.status)) {
      throw new ApiError(400, 'Repair log can only be added when device is received or in repair');
    }
    ...
    try {
      getIo().to(`ticket:${ticketId}`).emit('ticket:repair-log-added', { ticketId, logEntry: ticket.repairLog[ticket.repairLog.length - 1] });
    } catch(err) { console.error(err); }
